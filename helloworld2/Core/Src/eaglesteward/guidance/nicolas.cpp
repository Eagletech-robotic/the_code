...
void infinite_rectangle(config_t *config, input_t *input, output_t *output, state_t *func_state) {
    auto seq = sequence(
        [](input_t *lambda_input, output_t *lambda_output, state_t *current_state) {
            return gotoTarget(0.0, 0.0, 0.6, 0.0, 0.6, 0.6, 0, lambda_input, lambda_output, current_state);
        },
        [](input_t *lambda_input, output_t *lambda_output, state_t *current_state) {
            return gotoTarget(0.6, 0.0, 0.6, 0.6, 0.0, 0.6, 1, lambda_input, lambda_output, current_state);
        },
        [](input_t *lambda_input, output_t *lambda_output, state_t *current_state) {
            return gotoTarget(0.6, 0.6, 0.0, 0.6, 0.0, 0.0, 2, lambda_input, lambda_output, current_state);
        },
        [](input_t *lambda_input, output_t *lambda_output, state_t *current_state) {
            return gotoTarget(0.0, 0.6, 0.0, 0.0, 0.6, 0.0, 3, lambda_input, lambda_output, current_state);
        },
        [](input_t *, output_t *, state_t *current_state) {
...
void calcul_position(state_t *current_state, input_t *input, config_t *config) {
    ...
    fusion_odo_imu_fuse(input->imu_accel_x_mss, input->imu_accel_y_mss, input->delta_yaw_deg, input->encoder_left,
                        input->encoder_right, config->time_step_ms / 1000.0, current_state->theta_deg, &delta_x_m,
                        &delta_y_m, &delta_theta_deg, alpha_orientation_ratio, TICKS_PER_REV, WHEEL_CIRCUMFERENCE_M,
                        WHEEL_BASE_M);
    current_state->x_m += delta_x_m;
    current_state->y_m += delta_y_m;
    current_state->theta_deg += delta_theta_deg;
    current_state->theta_deg = angle_normalize_deg(current_state->theta_deg);
