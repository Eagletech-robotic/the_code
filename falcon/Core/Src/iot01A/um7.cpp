...
int um7_decode(int current_byte) {
    ...
    case STATE_SNP:
        ...
        return false;
    case STATE_PT:
        state = STATE_DATA; // Next state will be READ_DATA.  Save address to memory. (eg 0x70 for a
                            // DREG_EULER_PHI_THETA packet)
...
/*
        Causes the UM7 to measure the gyro outputs and set the output trim registers to compensate for any non-zero
...
// ne marche pas
void um7_set_home_north_current(UART_HandleTypeDef *huart) { um7_set_home_north(huart, yaw); }
